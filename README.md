...
5. 左上のサイドパネル(設定 / 履歴 / 検索)
   ...
   - 履歴: 過去の議論の一覧と本文。題名をクリックで改名、✕ で削除(確認なし。5 秒間は「取り消す」で戻せる。議論中の会話は消せない)、Markdown をコピー。「もう一度」で同じ議題・モード・ターン数のまま先後を入れ替えて新しい議論を始める。停止・エラーで止まった会話には「続きから」が出て、保存済みの発言の次のターンから同じ会話に続ける
