...
2. 議論テーマを入力し(Enter で改行、Ctrl+Enter / ⌘+Enter でも開始できる)、モード(対立 / 協調 / ブレスト / 弁証法 / リレー創作 / 批評 / 対談 / 師弟 / 悪魔の代弁者 / クイズ。括弧は先攻 / 後攻の役割。起動時は常に「対立」)、先攻(どちらの AI から話すか)、▲▼ で最大ターン数(1 AI 発言 = 1 ターン)を決めて「開始」。「一時停止」「再開」「停止」で進行を制御する。
...
5. 左上のサイドパネル(設定 / 履歴 / 検索)
   ...
   - 履歴: 過去の議論の一覧と本文。題名をクリックで改名、✕ で削除(確認なし。5 秒間は「取り消す」で戻せる。議論中の会話は消せない)、Markdown をコピー
