...
![ロールプレイ進行中の画面。上半分が管理ペイン、下半分に ChatGPT と Gemini](docs/screenshots/02-roleplay.png)
...
上はロールプレイモード(警察官 vs 容疑者)の進行中。ほかの画面:

![起動直後。両ペインが ready になり、テーマを入れて開始できる](docs/screenshots/01-idle.png)
*起動直後。テーマを入れてモード・先攻・ターン数を選び「開始」*

![経過表示。役割と完了バッジ付きで、発言が台詞形式で並ぶ](docs/screenshots/03-transcript.png)
*議論が終わると経過表示が前面に出る(Markdown としてコピーできる)*

![経過表示を下 80% に広げた画面](docs/screenshots/04-transcript-80.png)
*管理ペイン下端をドラッグすると経過表示を広げて読める*
...
- **macOS**(13 Ventura 以降): `.dmg` を開き、`ChatGPT vs Gemini` を Applications にドラッグ。Apple シリコン用(arm64)と Intel 用(x64)がある。いまは**未署名**なので初回は「開発元を確認できない」または「壊れている」と言われる。システム設定 → プライバシーとセキュリティ → 「このまま開く」で起動できる(ターミナルなら `xattr -d com.apple.quarantine "/Applications/ChatGPT vs Gemini.app"`)。
